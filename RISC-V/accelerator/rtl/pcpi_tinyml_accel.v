...
// pcpi_tinyml_accel.v
// PicoRV32-style PCPI coprocessor wrapper for the 4x4 Q5.10 systolic-array
// matrix accelerator.  This is the canonical copy used by the RISC-V
// pipeline integration tests (RISC-V/pipeline_top/).
//
// Memory layout (packed 2 Q5.10 elements per 32-bit word):
//   A matrix : base_a  .. base_a+0x1C  (8 words, rs1 byte address)
//   B matrix : base_b  .. base_b+0x1C  (8 words, rs2 byte address)
//   C matrix : 0x200   .. 0x21C        (8 words, hardcoded)
//
// Cycle budget: 8 A-reads + 8 B-reads + 10 compute + 8 C-writes + overhead
...
module pcpi_tinyml_accel (
    ...
    // Custom instruction encoding (R-type, custom-0 opcode)
    ...
    reg [2:0]  state;
    reg [4:0]  elem_idx;   // counts 0..7 (8 packed words per matrix)
    reg [31:0] base_a;
    reg [31:0] base_b;
    ...
    wire insn_match;
    assign insn_match = (pcpi_insn[6:0]   == OPCODE_CUSTOM0) &&
                        (pcpi_insn[14:12] == FUNCT3_MATMUL)  &&
                        (pcpi_insn[31:25] == FUNCT7_MATMUL);

    // Sign-extend the first C element for pcpi_rd
    ...
    // Combinational memory bus drive
    always @(*) begin
        ...
        case (state)
            ...
            S_STORE_C: begin
                ...
                // Pack two Q5.10 elements per 32-bit word: lo=even, hi=odd
                accel_mem_wdata = {c_flat[(elem_idx*2 + 1)*16 +: 16],
                                   c_flat[(elem_idx*2    )*16 +: 16]};
            end
            default: begin end
        endcase
    end
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            case (state)
                ...
                // Read A: 8 words, unpack 2 elements per word
                S_LOAD_A: begin
                    if (accel_mem_ready) begin
                        a_flat[(elem_idx*2    )*16 +: 16] <= accel_mem_rdata[15:0];  // even element
                        a_flat[(elem_idx*2 + 1)*16 +: 16] <= accel_mem_rdata[31:16]; // odd  element
                        if (elem_idx == 5'd7) begin
                            elem_idx <= 5'd0;
                            state    <= S_LOAD_B;
                        end else begin
                ...
                // Read B: 8 words, unpack 2 elements per word
                S_LOAD_B: begin
                    if (accel_mem_ready) begin
                        b_flat[(elem_idx*2    )*16 +: 16] <= accel_mem_rdata[15:0];
                        b_flat[(elem_idx*2 + 1)*16 +: 16] <= accel_mem_rdata[31:16];
                        if (elem_idx == 5'd7) begin
                            elem_idx <= 5'd0;
                            state    <= S_KICK;
                        end else begin
                ...
                // Write C: 8 words, pack 2 elements per word
                S_STORE_C: begin
                    if (accel_mem_ready) begin
                        if (elem_idx == 5'd7) begin

...
The cycle counter \texttt{timeout\_cycles} is an integer incremented on every rising clock edge.
Counting begins at the cycle when \texttt{resetn} is de-asserted (CPU comes out of reset) and
stops when the CPU writes the sentinel value (\texttt{c[0]}) to address \texttt{0x00000000},
which the testbench detects on the memory bus.
The measured window therefore covers the complete firmware execution lifetime:

\begin{enumerate}
    \item \textbf{CPU boot phase:} 8 clock cycles of held reset followed by the \texttt{\_start} entry point (stack-pointer initialisation via a single \texttt{li sp, 0x3f0} instruction).
    \item \textbf{Input copy phase} (\texttt{copy\_inputs}): 32 word-writes that DMA the compile-time matrix constants \texttt{a\_init[16]} and \texttt{b\_init[16]} from the firmware binary into the shared memory region (\texttt{0x100}--\texttt{0x17C}). This phase is \emph{identical} across all three configurations.
    \item \textbf{Computation phase} (\texttt{run\_matmul}): the configuration-specific work --- a single PCPI custom instruction for the accelerator path, or the C triple-nested loop for the two software paths.
    \item \textbf{Sentinel write:} one 32-bit store of \texttt{c[0]} to address \texttt{0x00000000}, which terminates counting.
\end{enumerate}

This is captured by every testbench as:
...
Phases 1, 2, and 4 are fixed-cost and equal for all three configurations;
only Phase 3 differs, so the speedup ratios isolate computation overhead cleanly.
...
\subsection{Benchmark Fairness and Standardization}

\paragraph{Unified firmware source.}
All three configurations compile the \emph{same} C source file
(\texttt{firmware\_matmul\_unified.c}) using preprocessor flags
(\texttt{MATMUL\_MODE\_ACCEL} / \texttt{MATMUL\_MODE\_SW}) that select the
\texttt{run\_matmul} implementation at compile time.
Entry point, stack setup, \texttt{copy\_inputs}, and sentinel write are
bytecode-identical across all three binaries.

\paragraph{Identical input data.}
Matrix operands \texttt{a\_init[16]} and \texttt{b\_init[16]} are compile-time
constants embedded in every firmware binary.
Every configuration runs against the same bit-for-bit input values;
there is no runtime randomness.

\paragraph{Identical clock and reset conditions.}
All testbenches use the same free-running clock and a fixed 8-cycle reset
hold before \texttt{resetn = 1}.
The counter starts at the same architectural moment (CPU out of reset) for
every configuration.

\paragraph{Equal constant-overhead phases.}
The boot (Phase~1), input copy (Phase~2), and sentinel write (Phase~4)
execute the same instruction sequences and access the same memory addresses
in all three configurations.
Their contribution cancels out in any speedup ratio computation.

\paragraph{ISA difference is intentional, not a bias.}
The MUL-enabled baseline deliberately uses \texttt{rv32im} with firmware
compiled for \texttt{rv32im}; both the core and the compiler must agree or
the multiply instruction is not exercised.
This configuration represents the strongest realistic software competitor
for the accelerator and is reported separately so its advantage over plain
rv32i is also visible.

\paragraph{Automated, non-manual measurement.}
Cycle counts are extracted by parsing the \texttt{TB\_CYCLES} log line emitted
by every testbench; no values are read manually from waveforms.
The same automation script processes all three outputs, making the comparison
